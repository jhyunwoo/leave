...
> 기준일: 2026-08-04. 이 문서는 현재 저장소의 API 스키마와 Expo 의존성을 기준으로 한 제출 초안입니다. 스토어 콘솔 제출 직전에 프로덕션 빌드, SDK Privacy Report, 서버 로그 필드를 다시 대조해야 합니다.
...
휴가 제목·사유·메모, 표시명, 그룹 이름·설명이 같은 그룹 구성원에게 노출될 수 있으므로 두 스토어 모두 사용자 생성 콘텐츠와 사용자 간 상호작용을 `예`로 신고합니다. 자유 채팅이나 공개 피드는 없고, 위치 공유와 결제는 없습니다.
