...
// 부대원 더미 데이터
const members = [
  { name: "김민수", rank: "상병", c: "#9fe870" },
  { name: "이준호", rank: "일병", c: "#ffd11a" },
  { name: "박서준", rank: "병장", c: "#7cc0ff" },
  { name: "최우식", rank: "이병", c: "#ff9db1" },
  { name: "정해인", rank: "상병", c: "#c3a6ff" },
];
...
function calendarScreen() {
  // 7월 달력: 특정 날짜에 인원/초과 표시
  const leaveDays = {
    3: { n: 2, cap: 4, names: ["민수", "준호"] },
    4: { n: 2, cap: 4, names: ["민수", "준호"] },
    11: { n: 3, cap: 4, names: ["서준", "우식", "해인"] },
    14: { n: 5, cap: 4, over: true, names: ["민수", "준호", "서준", "+2"] },
    15: { n: 5, cap: 4, over: true, names: ["민수", "준호", "+3"] },
    16: { n: 5, cap: 4, over: true, names: ["서준", "우식", "+3"] },
    22: { n: 1, cap: 4, names: ["해인"] },
    23: { n: 3, cap: 4, names: ["민수", "서준", "우식"] },
  };
  ...
  for (let d = 1; d <= days; d++) {
    ...
    cells += `<div class="cell ${over ? "over" : ""}">
      ...
      <div class="cnt ${over?"over":""}">${info.n}/${info.cap}</div>` : ""}
    </div>`;
  }
  return `
  <div class="screen-root" style="background:${C.canvasSoft}">
    ...
    <div style="padding:8px 28px 0">
      <div style="display:flex;justify-content:space-between;align-items:flex-start">
        <div>
          <div style="font-size:26px;color:${C.mute};font-weight:700">제3보병사단 · 1대대</div>
  ...
  <div class="screen-root" style="background:${C.canvasSoft}">
    ...
    <div style="margin:20px 26px 0;background:${C.negative};border-radius:20px;padding:26px 28px;display:flex;align-items:center;gap:18px">
      ...
      <div>
        ...
        <div style="color:#ffe3e4;font-weight:600;font-size:24px;margin-top:2px">출타 5명 · 최대 4명 (전체의 1/3)</div>
      </div>
    </div>
    <div style="padding:26px 30px 0;font-weight:800;font-size:28px;color:${C.ink}">이 날 휴가 중인 부대원</div>
    <div style="padding:14px 26px 0;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${C.canvas};border-radius:18px;padding:20px 22px;display:flex;align-items:center;gap:18px">
        ${avatar(m,64)}
        <div style="flex:1">
          <div style="font-weight:800;font-size:28px;color:${C.ink}">${m.name} <span style="color:${C.mute};font-weight:700;font-size:22px">· ${m.rank}</span></div>
...
function unitScreen() {
  const units = [
    { n: "제3보병사단 1대대", m: 24, r: "1/3" },
    { n: "제1기갑여단 정비대", m: 12, r: "2/5" },
    { n: "수도방위사령부 통신대", m: 31, r: "1/4" },
  ];
  return `
  <div class="screen-root" style="background:${C.canvasSoft}">
    ${statusBar()}
    <div style="padding:14px 34px 0">
      <div style="font-size:44px;font-weight:800;color:${C.ink};letter-spacing:-1px">부대 찾기</div>
    </div>
    <div style="padding:20px 30px 0">
      <div style="background:${C.canvas};border-radius:16px;padding:24px 26px;display:flex;align-items:center;gap:16px;font-size:27px;color:${C.mute};font-weight:600">
        <span style="font-size:28px"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" style="vertical-align:-5px"><circle cx="10.5" cy="10.5" r="6.5" stroke="${C.mute}" stroke-width="2.4"/><line x1="15.5" y1="15.5" x2="21" y2="21" stroke="${C.mute}" stroke-width="2.6" stroke-linecap="round"/></svg></span> 부대 이름 검색…
      </div>
    </div>
    <div style="padding:22px 30px 0;display:flex;flex-direction:column;gap:14px">
      ...
      <div style="background:${C.canvas};border-radius:18px;padding:24px 24px;display:flex;align-items:center;gap:16px;${i===0?`outline:3px solid ${C.primary}`:""}">
        <div style="width:60px;height:60px;border-radius:14px;background:${C.primaryPale};display:flex;align-items:center;justify-content:center;font-size:30px">🛡️</div>
        <div style="flex:1">
          <div style="font-weight:800;font-size:27px;color:${C.ink}">${u.n}</div>
          <div style="color:${C.mute};font-size:22px;margin-top:2px">부대원 ${u.m}명 · 최대 출타율 ${u.r}</div>
        </div>
    ...
    <div style="padding:26px 30px 0">
      <div style="border:3px dashed #c4cabf;border-radius:18px;padding:28px;text-align:center;color:${C.body}">
        <div style="font-weight:800;font-size:27px;color:${C.ink}">＋ 새 부대 만들기</div>
        <div style="font-size:22px;margin-top:6px">최대 출타율을 비율로 설정 (예: 1 / 3)</div>
      </div>
    </div>
  </div>`;
}
...
function notifScreen() {
  const items = [
    { t: "출타율 초과 알림", b: "제3보병사단 1대대에서 7월 15일에 최대 출타 인원을 초과했습니다.", time: "방금", unread: true },
...
function profileScreen() {
  ...
  <div class="screen-root" style="background:${C.canvasSoft}">
    ...
    <div style="padding:40px 34px 0;display:flex;flex-direction:column;align-items:center">
      ...
      <div style="font-size:40px;font-weight:800;color:${C.ink};margin-top:20px">김민수</div>
      <div style="font-size:26px;color:${C.body};margin-top:4px">육군 · 상병</div>
    </div>
    <div style="padding:34px 30px 0">
      <div style="background:${C.canvas};border-radius:20px;padding:28px 28px">
        <div style="display:flex;justify-content:space-between;font-size:24px;font-weight:700;color:${C.body}">
          <span>복무 진행률</span><span style="color:${C.inkDeep};font-weight:800">61%</span>
        </div>
        <div style="height:20px;border-radius:999px;background:${C.canvasSoft};margin-top:14px;overflow:hidden">
          <div style="width:61%;height:100%;background:${C.primary};border-radius:999px"></div>
        </div>
        <div style="display:flex;gap:16px;margin-top:26px">
          <div style="flex:1;background:${C.primaryPale};border-radius:16px;padding:22px;text-align:center">
            <div style="font-size:22px;color:${C.inkDeep};font-weight:700">다음 진급 (병장)</div>
            <div style="font-size:38px;font-weight:800;color:${C.inkDeep};margin-top:6px">D-42</div>
          </div>
          <div style="flex:1;background:${C.primaryPale};border-radius:16px;padding:22px;text-align:center">
            <div style="font-size:22px;color:${C.inkDeep};font-weight:700">전역까지</div>
            <div style="font-size:38px;font-weight:800;color:${C.inkDeep};margin-top:6px">D-296</div>
          </div>
        </div>
      </div>
      <div style="background:${C.canvas};border-radius:20px;padding:8px 28px;margin-top:16px">
        ${[["입대일","2025-09-08"],["전역 예정","2027-03-07"],["소속 부대","제3보병사단 1대대"]].map((r,i)=>`
...
function tabletCalendarScreen() {
  const leaveDays = {
    3:{n:2,cap:4,nm:["민수","준호"]},4:{n:2,cap:4,nm:["민수","준호"]},
    11:{n:3,cap:4,nm:["서준","우식","해인"]},
    14:{n:5,cap:4,over:1,nm:["민수","+4"]},15:{n:5,cap:4,over:1,nm:["민수","+4"]},16:{n:5,cap:4,over:1,nm:["서준","+4"]},
    22:{n:1,cap:4,nm:["해인"]},23:{n:3,cap:4,nm:["민수","서준","우식"]},
  };
  ...
  for(let d=1;d<=days;d++){const info=leaveDays[d];const today=d===11;const over=info?.over;const dow=(first+d-1)%7;
    cells+=`<div class="tc ${over?"tover":""}"><div class="tdn ${today?"ttoday":""}" style="${dow===0&&!today?`color:${C.negative}`:""}">${d}</div>
      ${info?`${info.nm.slice(0,2).map(n=>`<span class="tchip ${over?"tcover":""}">${n}</span>`).join("")}<div class="tcnt ${over?"tcntover":""}">${info.n}/${info.cap}</div>`:""}</div>`;}
  const on=[members[0],members[1],members[2],members[3],members[4]];
  const titles=["포상휴가","정기휴가","청원휴가","위로휴가","연가"];
  return `
  <div class="screen-root" style="background:${C.canvasSoft};flex-direction:row;padding:0">
    <div style="flex:1.55;padding:48px 44px;display:flex;flex-direction:column">
      <div style="display:flex;justify-content:space-between;align-items:flex-start">
        <div><div style="font-size:26px;color:${C.mute};font-weight:700">제3보병사단 · 1대대</div>
    ...
    <div style="flex:1;background:${C.canvas};padding:48px 40px;display:flex;flex-direction:column;gap:0">
      ...
      <div style="margin-top:22px;background:${C.negative};border-radius:20px;padding:26px;display:flex;align-items:center;gap:16px">
        ...
        <div style="color:#ffe3e4;font-weight:600;font-size:23px">출타 5명 · 최대 4명</div></div>
      </div>
      <div style="font-weight:800;font-size:28px;color:${C.ink};margin:28px 0 4px">이 날 휴가 중인 부대원</div>
      <div style="display:flex;flex-direction:column;gap:12px;margin-top:12px">
        ${on.map((m,i)=>`<div style="background:${C.canvasSoft};border-radius:16px;padding:18px 20px;display:flex;align-items:center;gap:16px">
          ${avatar(m,58)}<div style="flex:1"><div style="font-weight:800;font-size:26px;color:${C.ink}">${m.name} <span style="color:${C.mute};font-weight:700;font-size:20px">· ${m.rank}</span></div>
...
function tabletUnitScreen() {
  const units=[{n:"제3보병사단 1대대",m:24,r:"1/3"},{n:"제1기갑여단 정비대",m:12,r:"2/5"},{n:"수도방위사령부 통신대",m:31,r:"1/4"}];
  ...
  <div class="screen-root" style="background:${C.canvasSoft};flex-direction:row;padding:0">
    <div style="flex:1;padding:48px 40px">
      <div style="font-size:52px;font-weight:800;color:${C.ink};letter-spacing:-1px">부대 찾기</div>
      <div style="background:${C.canvas};border-radius:16px;padding:22px 24px;margin-top:22px;color:${C.mute};font-size:26px;font-weight:600"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" style="vertical-align:-5px"><circle cx="10.5" cy="10.5" r="6.5" stroke="${C.mute}" stroke-width="2.4"/><line x1="15.5" y1="15.5" x2="21" y2="21" stroke="${C.mute}" stroke-width="2.6" stroke-linecap="round"/></svg> 부대 이름 검색…</div>
      <div style="display:flex;flex-direction:column;gap:14px;margin-top:22px">
        ${units.map((u,i)=>`<div style="background:${C.canvas};border-radius:18px;padding:22px;display:flex;align-items:center;gap:16px;${i===0?`outline:3px solid ${C.primary}`:""}">
          <div style="width:58px;height:58px;border-radius:14px;background:${C.primaryPale};display:flex;align-items:center;justify-content:center;font-size:28px">🛡️</div>
          <div style="flex:1"><div style="font-weight:800;font-size:26px;color:${C.ink}">${u.n}</div>
          <div style="color:${C.mute};font-size:21px">부대원 ${u.m}명 · 최대 출타율 ${u.r}</div></div>
