...
- 심사 계정에는 **공개 사용자 이름(@아이디)이 미리 설정돼 있어야 합니다.** 0023 이후 사용자 이름 없는
  계정은 로그인 직후 1회성 설정 화면을 먼저 만나고, 친구 찾기는 이름이 유일한 공개 식별자라
  상대 아이디 없이는 평가할 수 없습니다. 새 심사 계정을 만들면 온보딩의 사용자 이름 단계까지
  마친 상태로 넘기세요(`packages/shared/src/username.ts`의 규칙을 따릅니다).
