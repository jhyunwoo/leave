/**
 * 군 종류별 복무 기간과 계급 자동 진급.
 *
 * 사용처: 서버가 사용자·구성원을 직렬화할 때, 앱/웹의 프로필·구성원 목록.
 *
 * 계급은 저장하지 않고 입대일에서 매번 계산한다. 저장해 두면 진급일이 지나도
 * 갱신되지 않아 화면마다 다른 계급이 보이게 된다. 가입 시 입력한 계급은
 * "그 시점의 실제 계급"이 표준 진급표와 다를 수 있어 보정값으로만 쓴다.
 */
