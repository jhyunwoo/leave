...
### 홈 화면 지표 위젯은 안쪽 여백을 직접 주지 않는다

WidgetKit이 iOS 17부터 위젯 내용에 기본 여백 16pt를 이미 넣는다(`app.json`의
`contentMarginsDisabled`는 계속 **false**다). 그 위에 `padding`을 또 얹으면 158pt짜리
소형 위젯에서 쓸 수 있는 폭이 98pt까지 줄어 절반 넘게가 여백이 된다. 그래서 지표 위젯의
홈 화면 세 크기는 여백을 시스템에만 맡기고, `frame({ maxWidth, maxHeight, alignment:
"topLeading" })`으로 남은 영역을 가로·세로 다 차지한다. 꼬리 `Spacer`를 두지 않는 것도
같은 이유다 — Spacer의 기본 최소 간격이 값 글자가 쓸 높이를 또 먹는다.

되찾은 자리는 값 글자에 준다(소형 54pt·중형 60pt·대형 112pt, 전부 heavy). 값이 커진
만큼 `minimumScaleFactor`의 바닥도 함께 내려 둔다 — `D-65`는 크게 나오고
`3시간 20분 05초`(복귀 타이머)처럼 긴 값은 예전만큼 작아질 수 있어야 잘리지 않는다.
**글자 크기를 올릴 때 바닥을 같이 내리지 않으면 긴 값이 말줄임으로 잘린다.**

요약 위젯(`LeaveSummary`)은 `StaticConfiguration`이라 기본 여백이 없는 iOS 16.4에서도
뜨므로 자기 `padding`을 그대로 들고 있다. 지표 위젯은 iOS 17 미만에 아예 없어서(아래 참고)
시스템 여백에 기댈 수 있다.
